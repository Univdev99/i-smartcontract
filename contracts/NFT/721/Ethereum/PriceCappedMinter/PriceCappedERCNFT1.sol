...
contract PriceCappedERCNFT1 is ERC721, Ownable, AccessControl {
    ...
    {
        require(cap_ > 0, "PriceCappedERCNFT1: cap is 0");
        _cap = cap_;
     ...
     */
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "PriceCappedERCNFT1: not admin");
        _;
    }
    ...
    modifier onlyOperator() {
        require(hasRole(OPERATOR_ROLE, _msgSender()), "PriceCappedERCNFT1: not operator");
        _;
    }

    modifier onlyExistentToken(uint256 tokenId) {
        require(_exists(tokenId), "PriceCappedERCNFT1: nonexistent token");
        _;
    }

    modifier onlyTokenOwner(uint256 tokenId) {
        require(ownerOf(tokenId) == _msgSender(), "PriceCappedERCNFT1: not token owner");
        _;
    }
    ...
    function addOperator(
        ...
        onlyAdmin
    {
        require(!checkOperator(account), "PriceCappedERCNFT1: already operator");
    ...
    function removeOperator(
        ...
        onlyAdmin
    {
        require(checkOperator(account), "PriceCappedERCNFT1: not operator");
    ...
    {
        _tokenIds.increment();
        require(_tokenIds.current() <= _cap, "PriceCappedERCNFT1: cap overflow");
    ...
    function applyOffer(
        ...
        payable
    {
        require(msg.value == offers[offerId].price, "PriceCappedERCNFT1: not enough fund");
        require(offers[offerId].isOpen, "PriceCappedERCNFT1: offer closed");
