...
contract PriceCappedERCNFT3 is ERC721, Ownable, AccessControl {
    ...
    {
        require(cap_ > 0, "PriceCappedERCNFT3: cap is 0");
        _cap = cap_;
     ...
     */
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "PriceCappedERCNFT3: not admin");
        _;
    }
    ...
    modifier onlyOperator() {
        require(hasRole(OPERATOR_ROLE, _msgSender()), "PriceCappedERCNFT3: not operator");
        _;
    }

    modifier onlyExistentToken(uint256 tokenId) {
        require(_exists(tokenId), "PriceCappedERCNFT3: nonexistent token");
        _;
    }

    modifier onlyTokenOwner(uint256 tokenId) {
        require(ownerOf(tokenId) == _msgSender(), "PriceCappedERCNFT3: not token owner");
        _;
    }
    ...
    function addOperator(
        ...
        onlyAdmin
    {
        require(!checkOperator(account), "PriceCappedERCNFT3: already operator");
    ...
    function removeOperator(
        ...
        onlyAdmin
    {
        require(checkOperator(account), "PriceCappedERCNFT3: not operator");
        ...
        returns (uint256)
    {
        ...
        require(currentSupply <= _cap, "PriceCappedERCNFT3: cap overflow");
    ...
    {
        _tokenIds.increment();
        require(_tokenIds.current() <= _cap, "PriceCappedERCNFT3: cap overflow");
    ...
    function applyOffer(
        ...
        payable
    {
        require(msg.value == offers[offerId].price, "PriceCappedERCNFT3: not enough fund");
        require(offers[offerId].isOpen, "PriceCappedERCNFT3: offer closed");
