...
contract CappedBNFT is BEP721, Ownable, AccessControl {
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(BEP165, AccessControl) returns (bool) {
     ...
     */
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "CappedBNFT: not admin");
        _;
    }
    ...
    modifier onlyOperator() {
        require(hasRole(OPERATOR_ROLE, msg.sender), "CappedBNFT: not operator");
        _;
    }
    ...
    function setTokenURI(uint256 tokenId, string memory tokenURI) public virtual onlyOperator {
        require(_exists(tokenId), "CappedBNFT: URI set of nonexistent token");
