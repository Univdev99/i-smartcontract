...
/**
 * @dev External interface of AccessControl declared to support POL165 detection.
 */
...
/**
 * @dev Interface of the POL20 Permit extension allowing approvals to be made via signatures, as defined in
 * https://eips.ethereum.org/EIPS/eip-2612[EIP-2612].
 *
 * Adds the {permit} method, which can be used to change an account's POL20 allowance (see {IPOL20-allowance}) by
 * presenting a message signed by the account. By not relying on `{IPOL20-approve}`, the token holder account doesn't
 * need to send a transaction, and thus is not required to hold Ether at all.
 */
interface IPOL20Permit {
    /**
     * @dev Sets `value` as the allowance of `spender` over `owner`'s tokens,
     * given `owner`'s signed approval.
     *
     * IMPORTANT: The same issues {IPOL20-approve} has related to transaction
 ...
 * of elements in a mapping, issuing POL721 ids, or counting request ids.
 *
...
/**
 * @dev Implementation of the POL20 Permit extension allowing approvals to be made via signatures, as defined in
 * https://eips.ethereum.org/EIPS/eip-2612[EIP-2612].
 *
 * Adds the {permit} method, which can be used to change an account's POL20 allowance (see {IPOL20-allowance}) by
 * presenting a message signed by the account. By not relying on `{IPOL20-approve}`, the token holder account doesn't
 ...
 * _Available since v3.4._
 */
abstract contract POL20Permit is POL20, IPOL20Permit, BIP712 {
     ...
     *
     * It's a good idea to use the same `name` that is defined as the POL20 token name.
     */
    constructor(string memory name) BIP712(name, "1") {
    }

    /**
     * @dev See {IPOL20Permit-permit}.
     */
    function permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public virtual override {
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp <= deadline, "POL20Permit: expired deadline");
        ...
        require(signer == owner, "POL20Permit: invalid signature");
    ...
    /**
     * @dev See {IPOL20Permit-DOMAIN_SEPARATOR}.
     */
    // solhint-disable-next-line func-name-mixedcase
...
/**
 * @dev POL20 token with pausable token transfers, minting and burning.
 *
 * Useful for scenarios such as preventing trades until the end of an evaluation
 * period, or having an emergency switch for freezing all token transfers in the
 * event of a large bug.
 */
abstract contract POL20Pausable is POL20, Pausable {
    /**
     * @dev See {POL20-_beforeTokenTransfer}.
     *
     * Requirements:
     *
     * - the contract must not be paused.
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        require(!paused(), "POL20Pausable: token transfer while paused");
    }
}

contract POL20PermitPausable is POL20Permit, Ownable, POL20Pausable, AccessControl {
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");

    constructor(string memory name_, string memory symbol_) public POL20Permit(name_) POL20(name_, symbol_) {
     ...
     */
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "POL20PermitPausable: not admin");
        _;
    }
    ...
    modifier onlyOperator() {
        require(hasRole(OPERATOR_ROLE, _msgSender()), "POL20PermitPausable: not operator");
        _;
    }
     ...
     */
    modifier onlyPauser() {
        require(hasRole(PAUSER_ROLE, _msgSender()), "POL20PermitPausable: not pauser");
        _;
    }
    ...
    /**
     * @dev POL20Pausable._beforeTokenTransfer(from, to, amount) override.
     * @param from address
     * @param to address
     * @param amount uint256
     */
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal override(POL20, POL20Pausable) {
        super._beforeTokenTransfer(from, to, amount);
    }
