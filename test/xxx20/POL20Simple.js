const POL20Simple = artifacts.require('POL20Simple');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('POL20Simple', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'POL20Simple';
  const symbol = 'POL20Simple';

  beforeEach(async () => {
    minter = await POL20Simple.new(name, symbol, {from: alice});
  });
