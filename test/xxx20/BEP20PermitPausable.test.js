const BEP20PermitPausable = artifacts.require('BEP20PermitPausable');
...
contract('::BEP20PermitPausable', async accounts => {
  let minter;
  let chainId;
  const [alice, bob, carl] = accounts;
  const name = 'BEP20PermitPausable';
  const symbol = 'BEP20PermitPausable';
  const version = '1';

  beforeEach(async () => {
    minter = await BEP20PermitPausable.new(name, symbol, {from: alice});
    ...
    describe('Role: reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert BEP20PermitPausable: not admin'
        );
      });
    ...
    describe('Token: reverts if', async () => {
      it('Token: should not be able to mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert BEP20PermitPausable: not operator'
        );
      });
    });
  });
  ...
  describe('#Pause', async () => {
    ...
    it('Pause: should not be able to send transactions when paused', async () => {
      ...
      await truffleAssert.reverts(
        minter.mint(alice, 10, {from: bob}),
        'revert BEP20Pausable: token transfer while paused'
      );
    });
  });
