const AC20PermitPausable = artifacts.require('AC20PermitPausable');
...
contract('::AC20PermitPausable', async accounts => {
  let minter;
  let chainId;
  const [alice, bob, carl] = accounts;
  const name = 'AC20PermitPausable';
  const symbol = 'AC20PermitPausable';
  const version = '1';

  beforeEach(async () => {
    minter = await AC20PermitPausable.new(name, symbol, {from: alice});
    ...
    describe('Role: reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert AC20PermitPausable: not admin'
        );
      });
    ...
    describe('Token: reverts if', async () => {
      it('Token: should not be able to mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert AC20PermitPausable: not operator'
        );
      });
    });
  });
