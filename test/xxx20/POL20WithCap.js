const POL20WithCap = artifacts.require('POL20WithCap');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('POL20WithCap', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'POL20WithCap';
  const symbol = 'POL20WithCap';
  const cap = 500;

  beforeEach(async () => {
    minter = await POL20WithCap.new(name, symbol, cap, {from: alice});
  });
