const POL20PermitPausable = artifacts.require('POL20PermitPausable');
...
contract('::POL20PermitPausable', async accounts => {
  let minter;
  let chainId;
  const [alice, bob, carl] = accounts;
  const name = 'POL20PermitPausable';
  const symbol = 'POL20PermitPausable';
  const version = '1';

  beforeEach(async () => {
    minter = await POL20PermitPausable.new(name, symbol, {from: alice});
    ...
    describe('Role: reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert POL20PermitPausable: not admin'
        );
      });
    ...
    describe('Token: reverts if', async () => {
      it('Token: should not be able to mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert POL20PermitPausable: not operator'
        );
      });
    });
  });
  ...
  describe('#Pause', async () => {
    ...
    it('Pause: should not be able to send transactions when paused', async () => {
      ...
      await truffleAssert.reverts(
        minter.mint(alice, 10, {from: bob}),
        'revert POL20Pausable: token transfer while paused'
      );
    });
  });
