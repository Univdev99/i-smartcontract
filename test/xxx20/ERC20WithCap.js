const ERC20WithCap = artifacts.require('ERC20WithCap');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('ERC20WithCap', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'ERC20WithCap';
  const symbol = 'ERC20WithCap';
  const cap = 500;

  beforeEach(async () => {
    minter = await ERC20WithCap.new(name, symbol, cap, {from: alice});
  });
  ...
  describe('Token', async () => {
    ...
    it('Token: should not be able to mint when cap overflow', async () => {
      await truffleAssert.reverts(
        ...
        'revert ERC20WithCap: cap overflow'
      );
    });
  });
