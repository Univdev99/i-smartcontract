const AC20Simple = artifacts.require('AC20Simple');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('AC20Simple', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'AC20Simple';
  const symbol = 'AC20Simple';

  beforeEach(async () => {
    minter = await AC20Simple.new(name, symbol, {from: alice});
  });
