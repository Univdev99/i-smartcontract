const AC20WithCap = artifacts.require('AC20WithCap');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('AC20WithCap', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'AC20WithCap';
  const symbol = 'AC20WithCap';
  const cap = 500;

  beforeEach(async () => {
    minter = await AC20WithCap.new(name, symbol, cap, {from: alice});
  });
