const BEP20Simple = artifacts.require('BEP20Simple');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('BEP20Simple', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'BEP20Simple';
  const symbol = 'BEP20Simple';

  beforeEach(async () => {
    minter = await BEP20Simple.new(name, symbol, {from: alice});
  });
