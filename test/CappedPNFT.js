const CappedPNFT = artifacts.require('CappedPNFT');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('CappedPNFT', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'Polygon BEP721 Capped';
  const symbol = 'CappedPNFT';
  const cap = 4000;

  beforeEach(async () => {
    minter = await CappedPNFT.new(name, symbol, cap, {from: alice});
  });
