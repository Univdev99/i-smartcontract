const PriceCappedACNFT2 = artifacts.require('PriceCappedACNFT2');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('::PriceCappedACNFT2', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'PriceCappedACNFT2';
  const symbol = 'PriceCappedACNFT2';
  const cap = 4000;
  const tokenPrice = 1000;

  beforeEach(async () => {
    minter = await PriceCappedACNFT2.new(name, symbol, cap, tokenPrice, {from: alice});
  });
    ...
    describe('reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedACNFT2: not admin'
        );
      });
    ...
    describe('reverts if', async () => {
      it('NFT: mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedACNFT2: not operator'
        );
      });
    });
  });
