const PriceCappedPNFT1 = artifacts.require('PriceCappedPNFT1');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('::PriceCappedPNFT1', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'PriceCappedPNFT1';
  const symbol = 'PriceCappedPNFT1';
  const cap = 4000;

  beforeEach(async () => {
    minter = await PriceCappedPNFT1.new(name, symbol, cap, {from: alice});
  });
    ...
    describe('reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedPNFT1: not admin'
        );
      });
    ...
    describe('reverts if', async () => {
      it('NFT: mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedPNFT1: not operator'
        );
      });
    });
  });
