const PriceCappedPNFT3 = artifacts.require('PriceCappedPNFT3');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('::PriceCappedPNFT3', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'PriceCappedPNFT3';
  const symbol = 'PriceCappedPNFT3';
  const cap = 4000;
  ...
  beforeEach(async () => {
    minter = await PriceCappedPNFT3.new(name, symbol, cap, minimumTokenPrice, ranges, {from: alice});
  });
    ...
    describe('reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedPNFT3: not admin'
        );
      });
    ...
    describe('reverts if', async () => {
      it('NFT: mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedPNFT3: not operator'
        );
      });
    });
  });
