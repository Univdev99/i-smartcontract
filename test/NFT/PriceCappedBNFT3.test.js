const PriceCappedBNFT3 = artifacts.require('PriceCappedBNFT3');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('::PriceCappedBNFT3', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'PriceCappedBNFT3';
  const symbol = 'PriceCappedBNFT3';
  const cap = 4000;
  ...
  beforeEach(async () => {
    minter = await PriceCappedBNFT3.new(name, symbol, cap, minimumTokenPrice, ranges, {from: alice});
  });
    ...
    describe('reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedBNFT3: not admin'
        );
      });
    ...
    describe('reverts if', async () => {
      it('NFT: mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedBNFT3: not operator'
        );
      });
    });
  });
