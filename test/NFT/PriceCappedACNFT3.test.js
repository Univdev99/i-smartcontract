...
const PriceCappedACNFT3 = artifacts.require('PriceCappedACNFT3');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('::PriceCappedACNFT3', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'PriceCappedACNFT3';
  const symbol = 'PriceCappedACNFT3';
  const cap = 4000;
  ...
  beforeEach(async () => {
    minter = await PriceCappedACNFT3.new(name, symbol, cap, minimumTokenPrice, ranges, {from: alice});
  });
    ...
    describe('reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedACNFT3: not admin'
        );
      });
    ...
    describe('reverts if', async () => {
      it('NFT: mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedACNFT3: not operator'
        );
      });
    });
  });
