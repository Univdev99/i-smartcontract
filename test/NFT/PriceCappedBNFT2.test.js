const PriceCappedBNFT2 = artifacts.require('PriceCappedBNFT2');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('::PriceCappedBNFT2', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'PriceCappedBNFT2';
  const symbol = 'PriceCappedBNFT2';
  const cap = 4000;
  const tokenPrice = 1000;

  beforeEach(async () => {
    minter = await PriceCappedBNFT2.new(name, symbol, cap, tokenPrice, {from: alice});
  });
    ...
    describe('reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedBNFT2: not admin'
        );
      });
    ...
    describe('reverts if', async () => {
      it('NFT: mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedBNFT2: not operator'
        );
      });
    });
  });
