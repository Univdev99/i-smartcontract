const CappedACNFT = artifacts.require('CappedACNFT');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('CappedACNFT', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'Avalanche ERC721 Capped';
  const symbol = 'CappedACNFT';
  const cap = 4000;

  beforeEach(async () => {
    minter = await CappedACNFT.new(name, symbol, cap, {from: alice});
  });
  ...
  describe('NFT', async () => {
    ...
    it('NFT: should not be able to mint a new token by non-operator', async () => {
      await truffleAssert.reverts(
        ...
        'revert CappedACNFT: not operator'
      );
    });
