const PriceCappedERCNFT2 = artifacts.require('PriceCappedERCNFT2');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('::PriceCappedERCNFT2', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'PriceCappedERCNFT2';
  const symbol = 'PriceCappedERCNFT2';
  const cap = 4000;
  const tokenPrice = 1000;

  beforeEach(async () => {
    minter = await PriceCappedERCNFT2.new(name, symbol, cap, tokenPrice, {from: alice});
  });
    ...
    describe('reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedERCNFT2: not admin'
        );
      });
    ...
    describe('reverts if', async () => {
      it('NFT: mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedERCNFT2: not operator'
        );
      });
    });
  });
