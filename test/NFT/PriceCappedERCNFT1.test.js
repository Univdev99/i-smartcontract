const PriceCappedERCNFT1 = artifacts.require('PriceCappedERCNFT1');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('::PriceCappedERCNFT1', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'PriceCappedERCNFT1';
  const symbol = 'PriceCappedERCNFT1';
  const cap = 4000;

  beforeEach(async () => {
    minter = await PriceCappedERCNFT1.new(name, symbol, cap, {from: alice});
  });
    ...
    describe('reverts if', async () => {
      it('Role: add operator by non-admin', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedERCNFT1: not admin'
        );
      });
    ...
    describe('reverts if', async () => {
      it('NFT: mint a new token by non-operator', async () => {
        await truffleAssert.reverts(
          ...
          'revert PriceCappedERCNFT1: not operator'
        );
      });
    });
  });
