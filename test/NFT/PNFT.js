const PNFT = artifacts.require('PNFT');
const { expect, assert } = require('chai');
const truffleAssert = require('truffle-assertions');

contract('PNFT', async accounts => {
  let minter;
  const [alice, bob, carl] = accounts;
  const name = 'Polygon POL721';
  const symbol = 'PNFT';

  beforeEach(async () => {
    minter = await PNFT.new(name, symbol, {from: alice});
  });
